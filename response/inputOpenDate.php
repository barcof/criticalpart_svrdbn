<?php
  ...
  $dryopenpart = isset($_REQUEST['dryopenpart']) ? $_REQUEST['dryopenpart'] : "";
  ...
  // +============================+
  try {
    $chkexp = $conn->Execute(" SELECT selflife, floorlife FROM tb_ctrlopen WHERE partno = '{$dryopenpart}' ");
    ...
    elseif ($getdate > $getselflife) {
      ...
      if (($getdate > $getfloorlife) && $getfloorlife != ''){
        ...
        // echo "EXEC insertDryPart_New '{$dryopenpart}','{$nik}','{$scancode}'";
        $sql = $conn->Execute(" EXEC insertDryPart_New '{$dryopenpart}','{$nik}','{$scancode}' ");
  ...
  switch ($var_msg) {
    ...
    case 2:
      echo "{
        'success': false,
        'msg': '<h3 style=\"color:#b71c1c;text-align:center\">UNFORTUNATELY THIS PART ALREADY EXPIRED !<br> PLEASE DO BAKING PROCEDURE</h3><br>'
      }";
    break;
    ...
    case 4:
      echo "{
        'success': false,
        'msg' : '<h3 style=\"text-align:center\">PART ALREADY OPEN</h3>'
      }";
    break;
